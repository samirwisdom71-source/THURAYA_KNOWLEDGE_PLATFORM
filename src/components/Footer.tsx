...
export function Footer({locale,siteName,tagline}:{locale:Locale;siteName:string;tagline?:string}) { const t=ui[locale]; return <footer className="siteFooter"><div className="container footerGrid">
  <div><div className="footerBrand"><img className="brandLogo" src={BRAND_LOGO} alt={siteName}/><div><b>{siteName}</b><p>{tagline|| (locale==='ar'?'منصة شخصية للمعرفة والاستدامة والأثر المجتمعي.':'A personal platform for knowledge, sustainability and community impact.')}</p></div></div></div>
